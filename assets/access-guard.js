(() => {
  ...
  function storageSet(storage, key, value) { try { storage.setItem(key, value); return true; } catch { return false; } }
  function storageRemove(storage, key) { try { storage.removeItem(key); } catch {} }
  function readStoredHash() {
    const keys = [STORAGE_KEY, ...LEGACY_KEYS];
    for (const storage of [localStorage, sessionStorage]) for (const key of keys) {
      const value = storageGet(storage, key);
      if (value === ACCESS_HASH) { if (key !== STORAGE_KEY) storageSet(localStorage, STORAGE_KEY, value); return value; }
    }
    return '';
  }
  function hasAccess() { return readStoredHash() === ACCESS_HASH; }
  function writeAccess(hash) { if (!storageSet(localStorage, STORAGE_KEY, hash)) storageSet(sessionStorage, STORAGE_KEY, hash); }
  function clearAccess() { for (const storage of [localStorage, sessionStorage]) { storageRemove(storage, STORAGE_KEY); LEGACY_KEYS.forEach((key) => storageRemove(storage, key)); } }
  async function sha256(value) {
    const bytes = new TextEncoder().encode(value);
    const digest = await crypto.subtle.digest('SHA-256', bytes);
    return [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, '0')).join('');
  }
  async function verifyAndStore(passcode) { const digest = await sha256(String(passcode || '').trim()); if (digest !== ACCESS_HASH) return false; writeAccess(digest); return true; }
  function currentRedirectValue() {
    const root = new URL(rootHref); const current = new URL(window.location.href);
    if (current.origin === root.origin && current.pathname.startsWith(root.pathname)) return current.href.slice(root.href.length) || './';
    return `${current.pathname}${current.search}${current.hash}`;
  }
  function portalUrlWithRedirect() { const url = new URL(rootHref); const redirect = currentRedirectValue(); if (redirect && redirect !== './') url.searchParams.set('redirect', redirect); return url.href; }
  function redirectTarget() {
    const raw = new URLSearchParams(window.location.search).get('redirect'); if (!raw) return '';
    const root = new URL(rootHref); const target = new URL(raw, root);
  ...
  function bootstrapCreatorPipeline() {
    ...
    if (!document.querySelector('script[data-gucc-creator-pipeline]')) {
      const bridge = document.createElement('script'); bridge.type = 'module'; bridge.src = new URL('assets/creator-pipeline-bridge.mjs?v=6', rootHref).href; bridge.dataset.guccCreatorPipeline = 'true'; document.head.appendChild(bridge);
    }
  ...
  window.GuccAccess = { hasAccess, verifyAndStore, clearAccess, redirectTarget, guardPage };
  const guardEnabled = script?.dataset.guard === 'true';
  const mayRender = guardEnabled ? guardPage() : true;
  if (mayRender) { bootstrapGlobalShell(); bootstrapCreatorPipeline(); }
})();
