...
import { attachObservationSummaries } from "./creator-file-observation-core.mjs";
...
function readLocalProjects() {
  try { const store = JSON.parse(localStorage.getItem(PRODUCTION_STORAGE_KEY) || "null"); return Array.isArray(store?.projects) ? store.projects : []; }
  catch { return []; }
}
...
function projectHref(projectId, route = "production") { const base = route === "publish" ? "./apps/publishing-console/" : "./apps/video-workspace/production-system/"; return `${base}?project=${encodeURIComponent(projectId)}`; }
function formatDate(value) {
  if (!value) return "未设置"; const date = new Date(value); if (Number.isNaN(date.getTime())) return String(value);
...
function lockPills(locks = {}) { return [["contentLock", "C"], ["scriptLock", "S"], ["audioLock", "A"], ["pictureLock", "P"]].map(([key, label]) => `<span class="creator-lock ${locks[key] ? "is-on" : ""}" title="${key}">${label}</span>`).join(""); }
