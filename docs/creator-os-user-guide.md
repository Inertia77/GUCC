...
先记住：**GUCC 不是云盘。** 真正的音频、视频、录屏和剪辑工程仍放在本机 Workspace。

每一行现在分成三件事：

- **标准位置**：GUCC 期望这个 Artifact 在项目目录里的位置。
- **项目状态**：这个 Artifact 是否已经纳入当前项目并标记 Ready / Missing。
- **本机状态**：Local Agent 是否真的在本机标准位置看到了文件。

例如 `AUDIO_MASTER.wav`：

- 本机状态显示 **「✓ 已找到」**：Local Agent 已经确认文件存在。
- 显示 **「⚠ 未找到」**：这台设备以前见过它，但现在标准位置没有文件。
- 显示 **「○ 尚未验证」**：Local Agent 还没有可靠观察结果。

注意：**本机已找到 ≠ 自动变成项目 Ready。** Local Agent 只证明物理文件存在，不会替你跨过项目确认或 Human Lock。
...
- Markdown / JSON / CSV / SRT：点 **「导入内容」**。GUCC 会读取文本内容，保存到当前 Project 状态，并随正常项目同步保存。
- WAV / MP4 等大型媒体：点 **「登记本地文件」**。GUCC 只记录文件名、大小和项目 Ready 信息，**不会上传音频 / 视频文件本体**。
- 轻量文本已有内容时，**「导出文本」** 只会把 GUCC 当前保存的文本内容导出成文件；它不是从云端下载真实媒体。

如果 Local Agent 已经显示 **「✓ 已找到」**，通常不需要为了“让 GUCC 看见文件”再点手工登记。只有需要把该文件正式纳入项目状态，或把文本内容重新读入项目时才使用手工操作。
