...
目标：
我会粘贴游戏版本更新公告、活动公告、前瞻整理、官方链接或截图识别文本。请把其中所有需要在限定时间内参与或完成的活动整理成可以直接粘贴进 Google Sheets 的 TSV 文本。

我的 Google Sheets 固定表头如下，必须严格按照这个顺序输出：

Title	MyDDL	Type	Status	Notes	Links
...
7. 限时网页活动。
8. 限时赛季或轮换玩法。
9. 有明确截止时间、需要完成活动内容才能获得奖励的任务。
10. 其他可能错过奖励或限定内容的活动。

最重要的原则：

只要属于限定、限时活动，并且适合整理成一个待完成任务，就优先输出。
...
1. 所有角色卡池信息。
2. 所有武器、光锥、音擎、装备卡池信息。
3. 所有寻访、跃迁、祈愿、申领及概率提升信息。
4. 所有签到活动。
5. 所有登录奖励。
6. 所有邮件奖励、维护补偿和问题修复补偿。
7. 所有上线即可领取、点击即可领取或免费直接领取的奖励。
8. 所有兑换码。
9. 永久开放的主线、支线、系统功能和常驻内容。
10. 单纯的新角色、新武器、新地图、新敌人介绍。
11. 系统优化、平衡调整和问题修复。

除上述明确排除项外，不要过度筛选。拿不准是否属于值得记录的限时活动时，优先保留，我可以之后手动删除。
...
游戏简称 + 版本号或版本名 +「活动名称」+ 活动类型

例如：

终末地「北观禁土」限时区域活动
鸣潮3.5「虚域危局·象限延展」挑战活动
崩铁4.4「命运/银河铁道之夜」联动活动
绝区零3.0「初升虚狩回归」限时活动

标题应简洁，能够直接看出游戏、活动名称和活动类型。
...
3. 不填写具体时刻。
4. 具体结束时刻写入 Notes。
5. 如果只写“版本结束前”“下次版本更新维护前”等，无法确定具体日期，则填写：

待确认

6. 不要自行推测日期。
7. 如果一个活动有两个独立活动周期，拆成两行。
8. 已经过期的活动不要输出。
...
1. 活动已经开始且尚未结束：01-推进
2. 活动尚未开始：02-等待
3. 长期固定轮换、确实需要长期保留的事项：00-固定
4. 普通版本活动一般使用01-推进或02-等待。
5. 按公告中的服务器时间判断。
6. 公告没有注明时区时，默认按国服时间，即UTC+8判断。

【Notes】

简洁写明：

活动类型；活动时间；需要完成的主要内容；重要奖励或限定奖励

例如：

限时挑战；活动时间：2026/07/30 12:00-2026/08/13 04:00（服务器时间）；完成系列挑战关卡；可获得限定头像框及活动奖励
...
1. 每个独立活动单独一行。
2. 同一个活动在公告中重复出现时，只保留一行。
3. 同一个活动的不同阶段共用一个截止日期时，可以合并，在 Notes 中写明分阶段开放时间。
4. 不同活动周期或不同截止日期应拆成多行。
5. 不要把同一个活动拆成“参加活动”“完成任务”“领取奖励”等多行。
6. 只记录活动任务本身，不单独记录领奖动作。

我将会粘贴公告内容在下面。直接输出符合以下表头的 TSV：
...
## 二游前瞻和版本情报写入Calendar
```text
你是我的二游版本情报整理助手。请帮我查询最近主要二游的前瞻直播、版本更新、重要活动节点，并整理后写入我的 Google Calendar。

要求如下：

1. 查询范围以国服为主，优先关注以下游戏：
...
* 《明日方舟：终末地》
* 《异环》
  如有其他近期热度很高、值得关注的二游，也可以补充，但不要乱加冷门无关游戏。

2. 信息来源优先级：

* 官方网站
* 官方公告
* 米游社 / 库街区 / TapTap 官方动态 / B站官方号
* 可信游戏媒体或社区整理
  如果信息来自非官方来源，必须明确标注“非官方”或“待确认”。

3. 需要整理的信息包括：

* 游戏名
* 事件类型：前瞻直播 / 版本更新 / 活动截止 / 卡池开启 / 重要版本节点
* 版本号和版本标题
* 国服时间
* 日本时间
* 信息来源
...
4. 写入 Google Calendar 的规则：

* 使用日本时间 Asia/Tokyo。
* 标题格式统一为：
  [二游] 游戏名 版本号 / 事件名
  如果是推算信息，标题前加：[预计]
* 描述里写清楚国服时间、版本名、来源、备注。
* 前瞻直播设置开播前 60 分钟和 10 分钟提醒。
* 版本更新设置提前 1 天提醒。
* 活动截止设置提前 1 天提醒。
* 如果同名事件已经存在，不要重复创建，先检查日历。
* 对不确定、不可靠、已经过期但没有参考价值的消息，不要写入日历。
* 如果只是已经结束的前瞻直播，除非我特别要求复盘，否则不要写入日历。
...
请务必联网检索最新消息，不要只凭记忆回答。信息不确定就直接说不确定，不要编。
```
