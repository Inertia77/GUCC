(() => {
  ...
  const officialGroups = [
    {
      key: '崩', description: '米游社官方 Wiki', routes: [
        ...
        ['敌人', 'https://bbs.mihoyo.com/sr/wiki/channel/map/17/23?bbs_presentation_style=no_header', '机制研究', '重点'],
    ...
    },
    {
      key: '终', description: '森空岛 Wiki 与工业参考', routes: [
        ...
        ['敌人', 'https://wiki.skland.com/endfield/catalog?typeMainId=1&typeSubId=3', '机制研究', '重点'],
    ...
    },
    {
      key: '绝', description: '米游社官方百科', routes: [
        ...
        ['敌人', 'https://baike.mihoyo.com/zzz/wiki/channel/map/2/65?mhy_presentation_style=fullscreen', '机制研究', '重点'],
    ...
    },
    {
      key: '鸣', description: '库街区官方 Wiki', routes: [
        ...
        ['敌人', 'https://wiki.kurobbs.com/mc/catalogue/list?fid=1099&sid=1158', '机制研究', '重点'],
  ...
  const state = { view: 'intel', game: 'all', query: '', sites: [] };
  ...
  function renderIntelCard(site, archived = false) {
    const keys = [...new Set(gameKeys(site))];
  ...
  function renderIntel() {
    ...
    const order = ['崩', '终', '绝', '鸣', '异'];
    active.sort((a, b) => {
      const aKey = order.indexOf(gameKeys(a)[0]);
      const bKey = order.indexOf(gameKeys(b)[0]);
      return (aKey - bKey) || (a.priority - b.priority) || a.name.localeCompare(b.name, 'zh-CN');
    });
    ...
    $('#clearSearch')?.addEventListener('click', () => { $('#sourceSearch').value = ''; state.query = ''; render(); });
  }
  ...
  function bindControls() {
    ...
    $$('.game-filter').forEach((button) => button.addEventListener('click', () => {
      ...
      $$('.game-filter').forEach((item) => item.classList.toggle('is-active', item === button));
      render();
    }));
