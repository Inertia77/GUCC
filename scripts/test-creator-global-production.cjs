...
assert.ok(html.includes('id="globalProduction"')); assert.ok(html.includes("creator-global-production-ui.mjs?v=7"), "Production must cache-bust Global UI changes");
assert.ok(html.includes("styles.css?v=5"), "Production must cache-bust Global layout changes");
assert.ok(html.includes("creator-ai-task-core.js?v=1"), "Production must load AI Task core");
assert.ok(html.includes("creator-ai-task-ui.mjs?v=1"), "Production must load AI Task UI");
assert.ok(html.includes("app.js?v=5"), "Production must cache-bust enriched Stage Prompt copy behavior");
...
// Async busy-state and write isolation are exercised by test-creator-global-ui.cjs.
...
assert.match(serviceWorker, /gucc-static-v28[\s\S]*gucc-runtime-v28/, "Global Production offline assets require a fresh Service Worker cache generation");
for (const offlineAsset of [
  ...
  "creator-file-observations.mjs?v=2", "creator-global-production-ui.mjs?v=7", "creator-ai-task-core.js?v=1", "creator-ai-task-ui.mjs?v=1", "creator-ux-simplification-v1.mjs?v=1", "production-system/styles.css?v=5", "production-system/app.js?v=5", "creator-pipeline-bridge.mjs?v=7", "creator-pipeline-core.mjs?v=2", "access-guard.js?v=8",
