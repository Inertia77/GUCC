...
assert.ok(!migration.includes("WP_GLOB_002 candidate migration"), "Canonical migration content must not retain the stale candidate header");
assert.ok(!migration.includes("This file is intentionally staged outside"), "Canonical migration content must not claim it is staged outside supabase/migrations");
assert.ok(!migration.includes("until Production assigns the canonical migration version"), "Canonical migration content must not describe a future Production-assigned version");
